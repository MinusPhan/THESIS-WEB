...
// Function to display folders in a table format
function displayFolders(folders, parentPath) {
    ...
    const accessHeader = document.createElement('th');
    accessHeader.textContent = 'Access';
    headerRow.appendChild(accessHeader);
    ...
    folders.forEach((folderName) => {
        ...
        accessButton.addEventListener('click', () => accessFolder(parentPath + folderName));
        accessCell.appendChild(accessButton);
    });

    // Append the table to the foldersList div
    foldersList.innerHTML = ''; // Clear previous content
    foldersList.appendChild(table);
}

// Function to access folders inside a folder
function accessFolder(folderPath) {
    const storageRef = storage.ref(folderPath);
    storageRef.listAll().then((res) => {
        if (res.prefixes.length > 0) {
            const folders = res.prefixes.map((folderRef) => folderRef.name);
            displayFolders(folders, folderPath + '/');
        }
...
// Function to display files in a table format
function displayFiles(files) {
    ...
    files.forEach((fileRef) => {
        const row = table.insertRow();
        const nameCell = row.insertCell();
        nameCell.textContent = fileRef.name;
    ...
    // Append the table to the filesList div
    foldersList.innerHTML = ''; // Clear previous content
    foldersList.appendChild(table);
}
...
function downloadFile(fileRef) {
    fileRef.getDownloadURL().then((url) => {
        ...
        downloadLink.download = fileRef.name;
        downloadLink.click();
    }).catch((error) => {
